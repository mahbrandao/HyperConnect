import { TrendingUp, FolderOpen, Users, CheckCircle2, ChevronRight, ArrowRight } from "lucide-react";
...
const activeCount = projects.filter(p => p.stage !== "Concluído").length;
const doneCount = projects.filter(p => p.stage === "Concluído").length;
const quoteCount = projects.filter(p => p.stage === "Orçamento").length;
...
export default function DashboardGerente() {
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-white">Painel Geral — HyperZ</h1>
          ...
          {STAGES.map((stage, i) => {
            ...
            return (
              <div key={stage} className="flex items-center flex-1 min-w-0">
                ...
                {i < STAGES.length - 1 && (
                  <ArrowRight size={14} className="text-[#333] flex-shrink-0 mx-1" />
                )}
              </div>
            );
          })}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#1e1e1e]">
              {["Projeto / Cliente", "Tipo", "Vendedor", "Instalador", "Etapa", "Valor", ""].map(col => (
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          ...
          const mine = projects.filter(p => p[field] === name);
          return (
